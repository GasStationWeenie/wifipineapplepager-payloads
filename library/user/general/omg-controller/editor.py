...
class PayloadEditor:
    ...
    def save(self, emit):
        ...
        try:
            ...
            raise ControllerError('Save incomplete or uncertain; not retried. Backup: ' + backup.name + '. ' + str(exc)) from exc
    ...
    def do(self, action, arg, emit):
        ...
        if action == 'edit-backup':
            ...
            self.store_backup(slot, data, emit)
        ...
        if action == 'edit-import-load':
            ...
            self.lines = source.splitlines(keepends=True)
            self.newline = '\r\n' if '\r\n' in source else '\n'
            emit('LOG', 'Imported ' + name + ' into draft. Review and choose Save.')
            return
