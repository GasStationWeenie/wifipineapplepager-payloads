...
view_slot() {
    ...
    while true; do
        ...
        WAIT_FOR_BUTTON_PRESS A
        nav=$(LIST_PICKER "Payload page $((page+1))/$PAGES" "Next" "Previous" "Back" "Next") || return
        case "$nav" in
...
edit_payload() {
    ...
    while true; do
        ...
        local lines=("${ITEMS[@]}")
        selected=$(LIST_PICKER "Edit payload $slot" "${lines[@]}" "Append line" "Save" "Discard" "Append line") || selected=Discard
        case "$selected" in
            Discard) request edit-discard; return ;;
            Save)
                answer=$(CONFIRMATION_DIALOG "Save changes to payload $slot?") || continue
                [ "$answer" = 1 ] || continue
            ...
            *)
                number=${selected%% |*}
                [[ "$number" =~ ^[0-9]+$ ]] || continue
                operation=$(LIST_PICKER "Line $number" "Edit line" "Insert before" "Delete line" "Back" "Edit line") || continue
                case "$operation" in
                    "Edit line")
        ...
        value=$(TEXT_PICKER "Payload line $number" "$EDIT_TEXT") || continue
        if [[ "$value" == *$'\n'* || "$value" == *$'\r'* ]]; then
...
create_payload() {
    ...
    choice=$(LIST_PICKER "Create payload $slot" "Write new" "Import .txt" "Restore Payload Backup" "Back" "Import .txt") || return
    case "$choice" in
...
payload_menu() {
    ...
    while true; do
        selected=$(LIST_PICKER "Payloads" "${choices[@]}" "Back" "${choices[0]}") || return
        [ "$selected" = Back ] && return
        slot=${selected%% |*}
        [[ "$slot" =~ ^[0-9]{3}$ ]] || return
        slot=$((10#$slot))
        ...
        action=$(LIST_PICKER "Payload $slot" "Execute" "View" "Edit" "Backup Payload" "Restore Payload Backup" "Clear Slot" "Back" "Execute") || continue
        case "$action" in
            View) view_slot "$slot" ;;
            ...
            "Restore Payload Backup"|"Clear Slot")
                if [ "$action" = "Restore Payload Backup" ]; then
                    ...
                    answer=$(CONFIRMATION_DIALOG "Clear payload $slot? A backup will be saved first.") || continue
                    [ "$answer" = 1 ] || continue
...
edit_descriptors() {
    ...
    while true; do
        selected=$(LIST_PICKER "Edit USB descriptors" "VID" "PID" "Manufacturer" "Product" "Serial" "Save" "Cancel" "VID") || return
        case "$selected" in
            Cancel) return ;;
            Save)
                if ! request descriptor-review; then pause_result; return; fi
                LOG "Reboot O.MG after saving to apply the new defaults."
                answer=$(CONFIRMATION_DIALOG "Save these USB descriptors?") || continue
                [ "$answer" = 1 ] || continue
        ...
        value=$(TEXT_PICKER "${names[$index]}" "${DESCRIPTORS[$index]}") || continue
...
descriptor_menu() {
    local selected
    while true; do
        selected=$(LIST_PICKER "USB descriptors" "View current" "Edit" "Back" "View current") || return
        case "$selected" in
...
restore_payload() {
    ...
    local backups=("${ITEMS[@]}")
    selected=$(LIST_PICKER "Restore Payload Backup" "${backups[@]}" "Back" "${backups[0]}") || return
    [ "$selected" = Back ] && return
    if ! request edit-restore-load "$selected"; then pause_result; return; fi
    if [ -n "${1:-}" ]; then
        if request edit-restore-target "$1"$'\t'"existing"; then
            answer=$(CONFIRMATION_DIALOG "Restore this backup to payload $1?") || answer=0
    ...
    answer=$(CONFIRMATION_DIALOG "Restore this backup to its original slot?") || { request edit-discard; return; }
    if [ "$answer" = 1 ]; then
        ...
        destination=$(LIST_PICKER "Restore to which slot?" "${destinations[@]}" "Back" "${destinations[0]}") || destination=Back
        if [ "$destination" != Back ]; then
            ...
            if [[ "$slot" =~ ^[0-9]{3}$ ]]; then
                ...
                if request edit-restore-target "$slot"$'\t'"$mode"; then
                    if [ "$mode" = new ]; then
                        answer=$(CONFIRMATION_DIALOG "Restore to empty slot $slot?") || answer=0
                    else
                        answer=$(CONFIRMATION_DIALOG "Replace payload $slot with this backup?") || answer=0
                    fi
                    [ "$answer" = 1 ] && run_action edit-save
                else
                    pause_result
...
extra_menu() {
    local choice answer
    while true; do
        choice=$(LIST_PICKER "Extra" "USB on" "USB off" "Jiggler on" "Jiggler off" "Connection timing" "Restore Payload Backup" "Back" "USB on") || return
        case "$choice" in
            "USB on") run_action usb-on ;;
            "USB off")
                answer=$(CONFIRMATION_DIALOG "USB off on O.MG?") || continue
...
while true; do
    choice=$(LIST_PICKER "O.MG Controller" \
    ...
    case "$choice" in
        ...
        "Extra") extra_menu ;;
        "Reboot")
            answer=$(CONFIRMATION_DIALOG "$choice on O.MG?") || continue
            [ "$answer" = "1" ] || continue
            run_action reboot
            ;;
        *) exit 0 ;;
